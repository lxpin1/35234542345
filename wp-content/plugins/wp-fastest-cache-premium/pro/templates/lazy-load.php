			...
			<div style="z-index: 1000; height: auto; position: relative; display: inline-block; width: 100%;" class="window-content">
				<div class="wpfc-cdn-pages-container">
					...
					<div wpfc-page="1" class="wiz-cont" style="display:none;">
                        ...
                        <p>Specify an image to be used as a placeholder while other images finish loading. <a target="_blank" href="http://www.wpfastestcache.com/premium/lazy-load-reduce-http-requests-and-page-load-time/#imageplaceholder"><img src="<?php echo plugins_url("wp-fastest-cache/images/info.png"); ?>" /></a></p>
                        <div class="wiz-input-cont">
			    			...
			    			<input type="text" placeholder="https://example.com/sample.jpg" name="custom-url" value="" class="api-key" style="width: 92%;display: none;"/>
			    			<span class="dashicons dashicons-trash" style="padding: 4px 0 0 4px;cursor: pointer;display: none;"></span>
					    </div>
					</div>
...
<script type="text/javascript">
	...
	function open_lazy_load_modal(){
		if(jQuery("div[id^='wpfc-modal-lazyload-']").length === 0){
			Wpfc_New_Dialog.dialog("wpfc-modal-lazyload", {
					next: function(){
						if(Wpfc_New_Dialog.clone.find("div.wiz-cont:visible").attr("wpfc-page") == 1){
							var custom_url_el = Wpfc_New_Dialog.clone.find("div.window-content input[name='custom-url']");

							if(custom_url_el.is(":visible")){
								if(custom_url_el.val().trim().length > 0){
									...
									custom_url_el.css("background-color", "white");
								}else{
									custom_url_el.css("background-color", "red");
								}
							}else{
								Wpfc_New_Dialog.hide_button("next");
								Wpfc_New_Dialog.show_button("back");
					...
					finish: function(e){
						...
						if(jQuery("#wpFastestCacheLazyLoad_placeholder").length > 0){
							var placeholder_value = Wpfc_New_Dialog.clone.find("div.window-content select.wpFastestCacheLazyLoad_placeholder").val();
							if(placeholder_value.match(/^custom/)){
								placeholder_value = "custom_" + Wpfc_New_Dialog.clone.find("input[name='custom-url']").val();
							}

							jQuery("#wpFastestCacheLazyLoad_placeholder").val(placeholder_value);
						}
						...
						if(jQuery("#wpFastestCacheLazyLoad_exclude_full_size_img").length){
							...
							if(exclude_full_size_img){
								jQuery("#wpFastestCacheLazyLoad_exclude_full_size_img").attr('checked', true);
							}else{
					...
					if(jQuery("#wpFastestCacheLazyLoad_placeholder").length == 0){
						...
						var placeholder_value = jQuery("#wpFastestCacheLazyLoad_placeholder").val();

						if(placeholder_value.match(/^custom/)){
							Wpfc_New_Dialog.clone.find("div.window-content select.wpFastestCacheLazyLoad_placeholder").val("custom");
							Wpfc_New_Dialog.clone.find("div.window-content select.wpFastestCacheLazyLoad_placeholder").hide();
							Wpfc_New_Dialog.clone.find("div.window-content input[name='custom-url']").show();
							Wpfc_New_Dialog.clone.find(".dashicons.dashicons-trash").show();
							Wpfc_New_Dialog.clone.find("div.window-content input[name='custom-url']").val(placeholder_value.replace(/^custom_/, ""));
						}else{
							Wpfc_New_Dialog.clone.find("div.window-content select.wpFastestCacheLazyLoad_placeholder").val(placeholder_value);
						}
					...
					// to show custom placeholder text
					Wpfc_New_Dialog.clone.find(".wpFastestCacheLazyLoad_placeholder").change(function(e){
						if(jQuery(e.target).val() == "custom"){
							Wpfc_New_Dialog.clone.find("input[name='custom-url']").show();
							Wpfc_New_Dialog.clone.find(".dashicons.dashicons-trash").show();
							jQuery(e.target).hide();
						}
					});

					// to hide custom placeholder text
					Wpfc_New_Dialog.clone.find(".dashicons.dashicons-trash").click(function(){
						Wpfc_New_Dialog.clone.find(".wpFastestCacheLazyLoad_placeholder").val("default");
						Wpfc_New_Dialog.clone.find(".wpFastestCacheLazyLoad_placeholder").show();
						Wpfc_New_Dialog.clone.find("input[name='custom-url'], .dashicons.dashicons-trash").hide();
					});

					// to disable the space for the custom placeholder
					Wpfc_New_Dialog.clone.find("div.window-content input[name='custom-url']").keypress(function(e){
						if(e.which == 32){
							return false;
						}
					});
