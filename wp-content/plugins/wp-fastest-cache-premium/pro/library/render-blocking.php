	...
	class WpFastestCacheRenderBlocking{
		...
		public function set_js(){
			...
			foreach ($script_tag as $key => $value) {
				...
				if(preg_match("/data-cfasync\=[\"\']false[\"\']/i", $value["text"])){
					continue;
				}
				...
				*/
				...
				if(preg_match("/<script[^\>]+text\/(template|html|mf)[^\>]+/i", $value["text"])){
					continue;
				}
				...
				*/
				...
				// <script src="//tp.media/content?currency=usd&promo_id=4044&shmarker=411367&campaign_id=100&trs=210595&target_host=%2Fsearch&locale=en&limit=5&powered_by=false&destination=LAS" charset="utf-8"></script>
				if(preg_match("/tp\.media\/content/i", $value["text"])){
					continue;
				}
				...
				if($value["text"] && preg_match("/^<script/i", $value["text"])){
					if(preg_match("/gravatar\.com\/js\/gprofiles\.js/i", $value["text"]) || 
						...
						if(preg_match("/\}\s*\)\s*\;\s*<\/script>$/i", $value["text"])){
							...
							$value["text"] = preg_replace("/(<script[^\>]*>)/i", "$1"."(function ".$jqIsReady_func_name."(){if(typeof jQuery === \"undefined\"){setTimeout(".$jqIsReady_func_name.", 100);}else{", $value["text"]);
	    				...
	    				//<script>var dswcp_pre__=window._;</script>
	    				if(preg_match("/var\s+dswcp_pre__\s*\=\s*window\.\_\;/i", $tmp)){
	    					continue;
	    				}
	    				...
	    				*/
	    				...
	    				if(preg_match("/wp-data-js-after/i", $value["text"])){
		...
		public function defer_load_inline_js($script){
			...
			if(preg_match("/window\.RS_MODULES/i", $script)){
				// Revolution Slider
				if(preg_match("/var\s+tpj\s*\=\s*jQuery/i", $script)){
					return $script;
				}
			}

			if(preg_match("/wpforms-recaptcha-js-after/i", $script)){
				return $script;
			}
