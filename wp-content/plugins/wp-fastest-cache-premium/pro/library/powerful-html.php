<?php
	class WpFastestCachePowerfulHtml{
		...
		public function remove_type_attribute_for_js($script){
			...
			if(preg_match("/type\s*=\s*[\"\']\s*module\s*[\"\']/i", $script)){
				return $script;
			}

			// <script src="https://assets.adobedtm.com/acdc00762da2/fbdac6838208/launch-EN1345726ffe309bd307c69.min.js" type="text/plain" class="optanon-category-C006"></script>
			if(preg_match("/type\s*=\s*[\"\']\s*text\/plain\s*[\"\']/i", $script)){
				return $script;
			}
		...
		public function eliminate_newline($start_string, $end_string, $tmp_html){
			...
			if(isset($list[0])){
				...
				foreach ($list as $key => $value) {
					if(preg_match("/(<script|<style|<textarea|<code)/i", $value["text"])){
						continue;
					}
					...
					if(preg_match("/<li\s+class\=\"tmcp-field-wrap\">/i", $value["text"])){
						continue;
					}
		...
		public function lazy_load($content){
			...
			$content = $lazy->video_to_lazyload($content, $this->inline_scripts);
		...
		public function remove_multi_line_comments_from_js($matches){
			...
			// /* Wrapped ".concat(t,"*/
			if(preg_match("/concat\s*\([a-z]+\s*\,\s*\"\s*\*\//i", $matches[0])){
				return $matches[0];
			}
			...
			if(preg_match("/\s*\;\s*q\s*\=\s*/i", $matches[0])){
				return $matches[0];
			}
		...
		public function remove_single_line_comments_from_js($matches){
			...
			if(preg_match("/^tsapp\:\/\/(send|\s)/", $matches[0])){
				return $matches[0];
			}

			// chrome-extension://gpdjojdkbbmdfjfahjcgigfpmkopogic/html/save.html
			if(preg_match("/^nsion\:\/\//", $matches[0])){
				return $matches[0];
			}
			...
			if(!empty($script_list)){
				foreach (array_reverse($script_list) as $key => $value) {
					...
					if(preg_match("/^<script[^\>\<]+src\=[^\>\<]+>/i", $script_tag) && !preg_match("/\/wpfc\-minified\//i", $script_tag)){
						...
						if(isset($src[1]) && preg_match("/".preg_quote($http_host, "/")."/i", $src[1])){
							...
							$cachFilePath = $GLOBALS['wp_fastest_cache']->getWpContentDir("/cache/wpfc-minified")."/".$js_file_name;

							if(!defined('WPFC_WP_CONTENT_URL')){
								$jsScript = content_url()."/".preg_replace("/.+\/(cache\/[^\/]+).*/", "$1", $cachFilePath)."/".$js_file_name;
							}else{
								$jsScript = WPFC_WP_CONTENT_URL."/".preg_replace("/.+\/(cache\/[^\/]+).*/", "$1", $cachFilePath)."/".$js_file_name;
							}
