<?php
include(DELLIN_PLUGIN_DIR."/inc/dellinClasses/DellinLogger.php");
new DellinLogger;
/**
 * Created by PhpStorm.
 * User: ehrapov
...
//        }
     ...
     * @return array
     */
    public static function iconvArray(&$inputArray,$oldEncoding,$newEncoding){
        //$outputArray=array();

        if(is_object($inputArray) || is_array($inputArray)) {
            foreach ($inputArray as $key => &$element){
                self::iconvArray($element, $oldEncoding, $newEncoding);
            }
        } elseif (is_string($inputArray)) {
            $inputArray = iconv($oldEncoding, $newEncoding, $inputArray);
        }

//        if (!empty((array) $inputArray)){
//            foreach ($inputArray as $key=>&$element){
//                if (is_array($element)){
//                    $element = self::iconvArray($element,$oldEncoding,$newEncoding);
//                }elseif(is_object($element)){
//                    //$element = (array) $element;
//                    $element = self::iconvArray($element,$oldEncoding,$newEncoding);
//                }elseif(is_string($element)) {
//                    $element = iconv($oldEncoding,
//                        $newEncoding,$element);
//
//                }
...
//        }
    ...
    protected static function sendCurl($url,$postData,$method="POST",$type="json"){
        ...
        if(!is_object($response) && isset($response['body']) && is_string($response['body'])){

            $result = $response['body'];
        } else {
            if(count($response->get_error_messages()) > 0){
                $result = [];
                $result['isError'] = true;
            ...
            if(DellinLogger::getProps('IO_CURL')){
                $data['bodyRequest'] = $postData;
                $data['bodyResponse'] = $result;
                if(self::$apiUrls['calculator'] == $url ||
                   self::$apiUrls['request'] == $url){
                    DellinLogger::write_io_message($url, $data);
                }
            }
    ...
    public static function  GetStreetKladr($arParams){
        ...
        if($cities == null){
            return $result = array('STATUS' => 'ERROR', 'BODY' => 'API is not available');
        }
        if(is_array($cities->cities) && empty($cities->cities)){
            return $result = array('STATUS' => 'ERROR', 'BODY' => 'Wrong city KLADR code');
        }
        $cityID = $cities->cities[0]->cityID;
        if($cityID == null){
            $result = array('STATUS' => 'ERROR', 'BODY' => 'City is not specified');
        }else{
            $data = array(
            ...
            if($response == null){
                $result = array('STATUS' => 'ERROR', 'BODY' => 'API is not available');
            }elseIf(isset($response['errors'])){
                $result = array('STATUS' => 'ERROR', 'BODY' => "API error: ".$response['errors']);
            }else{
                $result = $response;
                $result['STATUS'] = 'OK';
            }
    ...
    {
        $volume_in_meter = round($param_goods_volume_in_mm / (1000 * 1000 * 1000),3);
     ...
     * @return array
     */
    public static function GetVolumeArrayBySize($param_size_list, $sized_volume)
    {
